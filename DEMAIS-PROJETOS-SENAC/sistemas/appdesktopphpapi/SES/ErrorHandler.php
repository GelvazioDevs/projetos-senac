...
final class ErrorHandler {
    ...
    public function setError()
    {
        ...
        if (is_array($error)) {
//            $formatedMessage = sprintf(
//                $this->template, 
//                $error['message'],
//                $error['file'], 
//                $error['line'], 
//                $this->getErrorDescription($error['type']), PHP_VERSION);

            $formatedMessage = sprintf(
                $this->template,
                '<div class="alerta_grande erro">' . $error['message'] . '</div>',
                '<div class="alerta_grande info">' . $error['file']  . '</div>',
                '<div class="alerta_grande info">' . $error['line']  . '</div>',
                '<div class="alerta_grande info">' . $this->getErrorDescription($error['type']) . '</div>',
                '<div class="alerta_grande info">' . PHP_VERSION . '</div>'
            );

            
            $this->mostraErroRetorno($formatedMessage);
        }
    }
    ...
    public function mostraErroRetorno($formatedMessage){
        $oDados = new stdClass();
        $oDados->mensagem = "Ocorreram erros ao processar dados!";
        $oDados->trace    = SESModelMensagem::getCssMensagem() . $formatedMessage;
        $oDados->status   = SESModelMensagem::MENSAGEM_ERRO;
